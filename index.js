...
if (selectedArtist) {
  ...
  //ky variabli ketu eshte i paperdorur
  const totalItems = items.filter(
    (item) => item.artist === selectedArtist
  ).length;

  // perdore ate variablin totalItems tek ky rreshti poshte qe te dali psh 4/10 - pra 4 items te shitur nga 10 gjithsej
  // mund te vendosesh direkt ate shenjen / sepse po tregon nje string dhe nuk pjestohen numrat
...
const publishedItems = items.filter(
  (item) => item.isPublished === true && item.dateSold === null
);
